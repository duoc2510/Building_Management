                ...
                when {
                    ...
                    loadingUrl.startsWith("https://") && isBankingUrl(loadingUrl) -> {
                        return try {
                            ...
                            Toast.makeText(this@WebPayCartActivity, "Đang mở trong trình duyệt...", Toast.LENGTH_SHORT).show()
                            true
                        } catch (_: Exception) {
                            false
                        }
                    ...
                    !loadingUrl.startsWith("http") && !loadingUrl.startsWith("https") && isBankingScheme(loadingUrl) -> {
                        return try {
                            ...
                            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                            startActivity(intent)
                            Toast.makeText(this@WebPayCartActivity, "Đang chuyển đến app ngân hàng...", Toast.LENGTH_SHORT).show()
                            true
                        } catch (_: Exception) {
                            Toast.makeText(this@WebPayCartActivity, "Vui lòng cài đặt app ngân hàng để tiếp tục", Toast.LENGTH_LONG).show()
                            false
                        }
                    }
                    ...
                    !loadingUrl.startsWith("http") && !loadingUrl.startsWith("https") -> {
                        return try {
                            ...
                            Toast.makeText(this@WebPayCartActivity, "Không thể mở ứng dụng liên kết", Toast.LENGTH_SHORT).show()
                            true
                        }
                    }
                }
        ...
        onBackPressedDispatcher.addCallback(this, object : OnBackPressedCallback(true) {
            override fun handleOnBackPressed() {
                if (webView != null && webView!!.canGoBack()) {
                    ...
                    android.app.AlertDialog.Builder(this@WebPayCartActivity)
    ...
    private fun handlePaymentSuccess(url: String) {
        try {
            ...
            if (isValidSuccess) {
                    ...
                    webView?.postDelayed({
                        webView?.evaluateJavascript("window.location.href") { currentUrl ->
                            if (currentUrl.contains("myapp://payment-success")) {
                                // Gửi kết quả thành công về CartActivity
                                ...
                                saveCartHistoryToFirebase(orderCode, paymentLinkId)
                            }
                        }
                    }, 3000)
                    return
                ...
                // Trường hợp orderCode không null hoặc không cần delay
                val resultIntent = Intent().apply {
                    putExtra("payment_success", true)
                }
                setResult(Activity.RESULT_OK, resultIntent)
            ...
            if (url.startsWith("myapp://payment-success") || url.contains("PAID")) {
                saveCartHistoryToFirebase(null, null)
            }
        }
    }
    private fun saveCartHistoryToFirebase(orderCode: String?, paymentLinkId: String?) {
